  ...
  return (
    <div className="absolute bottom-12 left-12 w-[240px] bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.1)] border border-gray-100 p-4 z-50 transform origin-bottom-left transition-all text-gray-800">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <div
              ...
              className={`relative w-12 h-12 shrink-0 rounded-full bg-gray-100 flex items-center justify-center ring-1 ring-black/5 overflow-hidden ${isSelf ? 'cursor-pointer' : ''
                }`}>
              <img
            ...
            <div className="absolute top-full left-0 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-lg p-1.5 z-10 flex flex-col gap-0.5">
              {Object.values(USER_STATUS_CONFIG).map(({ label, color }) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-colors w-full text-left ${currentStatus === label ? 'bg-gray-100 text-gray-900' : 'hover:bg-gray-50 text-gray-600'
                    }`}>
      ...
      <div
        ...
        className={`relative group mt-2 bg-gray-50 rounded-xl p-3 pb-5 min-h-[52px] flex items-start border border-transparent overflow-hidden ${isSelf ? 'hover:bg-gray-100/80 cursor-pointer hover:border-gray-200' : ''
          }`}>
        {isSelf && isEditing ? (
          <input
            ref={inputRef}
