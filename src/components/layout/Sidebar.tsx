...
const STATUS_TO_SERVER: Record<UserStatus, string> = {
    "대화 가능": "online",
    "자리 비움": "away",
    "방해 금지": "busy",
    오프라인: "offline",
...
export function Sidebar() {
        ...
        const fetchProfile = async () => {
            try {
                ...
                const serverPresence =
                    response.data.presence?.status || response.data.status;
                if (serverPresence) {
                    const mappedStatus =
                        SERVER_TO_STATUS[serverPresence] || serverPresence;
    ...
    const handleLogout = async () => {
        ...
        try {
            ...
            console.warn(
                "백엔드 로그아웃 처리 중 에러 발생 (이미 만료된 토큰 등):",
                error,
            );
        } finally {
            // 1. 스토리지 정리
    ...
    const statusColor = USER_STATUS_CONFIG[status]?.color || "bg-gray-500";
        ...
        <aside className="flex w-16 shrink-0 flex-col items-center justify-between pb-4 bg-bg-canvas relative h-full">
            ...
            <div className="relative mt-auto" ref={menuRef}>
                ...
                >
                    ...
                    />
                    <div
                        className={`absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full ${statusColor} ring-2 ring-bg-canvas`}
                    />
                </button>
