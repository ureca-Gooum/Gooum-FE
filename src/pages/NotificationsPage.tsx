...
// 더미 텍스트 데이터
const dummyText1 = 'Cupcake ipsum dolor sit amet muffin sesame snaps caramels. Gingerbread chupa chups cupcake tiramisu croissant. Pastry apple pie halvah cheesecake candy tiramisu cake.';
...
const DUMMY_MESSAGES: Message[] = [
  { id: 'm1', roomId: 'r1', senderId: 'u1', senderName: 'Daichi Fukuda', content: createDummyDoc(dummyText1), time: '5/12, 9:15 AM', isMine: false, isDeleted: false },
  { id: 'm2', roomId: 'r1', senderId: 'me', senderName: '나', content: createDummyDoc(dummyText1), time: '5/12, 9:15 AM', isMine: true, isDeleted: false },
  { id: 'm3', roomId: 'r1', senderId: 'u1', senderName: 'Daichi Fukuda', content: createDummyDoc(dummyText2), time: '5/12, 9:16 AM', isMine: false, isDeleted: false },
];
      ...
      <MainPanel
        header={
          <div className="flex h-[63px] items-center gap-3 border-b border-border-default px-5">
            <Avatar
              seed={selectedNoti.id}
              imageUrl={undefined}
              presence="online"
              alt={selectedNoti.title}
              size={32}
            />
            ...
            <div className="flex items-center gap-5 ml-4">
              <button
                ...
                className={`relative text-[13px] transition-colors ${activeMainTab === '채팅' ? 'font-bold text-brand-primary' : 'font-medium text-fg-tertiary hover:text-fg-primary'
                  }`}
              >
                채팅
                {activeMainTab === '채팅' && (
                  <span className="absolute left-0 right-0 bg-brand-primary" style={{ bottom: '-21px', height: '2px' }} />
                )}
              </button>
              <button
                onClick={() => setActiveMainTab('파일')}
                className={`relative text-[13px] transition-colors ${activeMainTab === '파일' ? 'font-bold text-brand-primary' : 'font-medium text-fg-tertiary hover:text-fg-primary'
                  }`}
              >
                파일
                {activeMainTab === '파일' && (
              ...
              <button
                ...
                className={`relative flex items-center gap-1 text-[13px] transition-colors ${activeMainTab === 'AI 회의록' ? 'font-bold text-brand-primary' : 'font-medium text-fg-tertiary hover:text-fg-primary'
                  }`}
              >
                <Sparkles size={14} />
                AI 회의록
